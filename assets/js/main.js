...
console.log(replaceCSSComments('код без /*комментарий*/ комментов') === 'код без комментов'); // код без комментов
console.log(replaceCSSComments('код бе/*коммент1*/з ком/* коммент2 */ментов') === 'код без комментов'); // код без комментов
console.log(replaceCSSComments('код /*к1*/ без /* к2 */ коммент/*к3*/ов') === 'код без комментов'); // код без комментов
...
console.log(replaceHTMLComments('<!--коммент1--> код без комментов <!--коммент2-->') === 'код без комментов'); // код без комментов
console.log(replaceHTMLComments('<!--к1--> код <!-- к2 --><!-- к3 --> без <!-- к4 --> комментов') === 'код без комментов'); // код без комментов
console.log(replaceHTMLComments('код <!--к1--> без <!-- к2 --> комментов') === 'код без комментов'); // код без комментов
console.log('-----------------');
...
function validateFileType(str) {
  const reg = new RegExp(/(\.p.g$)|(\.j.g$)|(\.j..g$)/);
...
function checkYear(str) {
  const reg = new RegExp(/^[1-2][09]\d{2}$|^[1-2][019]\d0$/);
  return reg.test(str);
};
...
function checkEmail(mail) {
  const reg = new RegExp(/^[a-z\.\-]+\@[a-z]+\.[a-z]+$|^[a-z\.\-]+\@[a-z]+\.[a-z]+\.[a-z]+$/);
...
function checkDomainUrl(domen) {
  const reg = new RegExp(/^http:\/\/[a-zA-z\.\-\d\_]+\.[a-z]{2,4}$|^https:\/\/[a-zA-z\.\-\d\_]+\.[a-z]{2,4}$/);

  return reg.test(domen);
}
...
function createLinksFromDomains(str) {
  let reg = new RegExp( /* /((http:|https:)\/\/)([a-zA-Z\d_-]+\.[a-z]{2,3})*\s([a-zA-Z\d\-_]+)/g */ '(http:|https:)\\/\\/(?!:\\/\\/)([a-zA-Z0-9-_]+\\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\\.[a-zA-Z]{2,5}', 'gi');
